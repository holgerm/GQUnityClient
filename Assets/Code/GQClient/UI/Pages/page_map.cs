	...
	{ 
		...
		if (showMap) {
			...
			if (ConfigurationManager.Current.useMapOffline) {
				...
				// create an OSM tile layer
				OSMTileLayer localTileLayer = map.CreateLayer<OSMTileLayer> ("LocalTiles");
				//osmLayer.BaseURL = "http://a.tile.openstreetmap.org/";
				localTileLayer.BaseURL = pre + Application.streamingAssetsPath + "/mapTiles/";
				localTileLayer.TileImageExtension = ".jpg";

				layers.Add (localTileLayer);
