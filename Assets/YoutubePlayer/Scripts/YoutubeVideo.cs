...
public class YoutubeVideo : MonoBehaviour {
	...
	public string RequestVideo (string urlOrId, int expectedQuality) {
		...
		int bestQualityMissing = expectedQuality;
		...
		while ( enumerator.MoveNext() ) {
			Debug.Log("VIDEO INFO: " + enumerator.Current.VideoType.ToString() + " RES: " + enumerator.Current.Resolution.ToString());
			if ( enumerator.Current.VideoType == VideoType.Mp4 && bestQualityMissing > expectedQuality - enumerator.Current.Resolution ) {
				// we found a solution better than any before:
				Debug.Log("VIDEO SOLUTION FOUND.");
				video = enumerator.Current;
				bestQualityMissing = expectedQuality - enumerator.Current.Resolution;

				if ( bestQualityMissing <= 0 )
		...
		if ( video == null ) {
			Debug.Log("Video URL not valid: " + urlOrId);
			return null;
		}
		else
			Debug.Log("VIDEO VALID going to play it ...");
