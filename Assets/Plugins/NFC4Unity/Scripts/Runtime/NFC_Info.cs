...
namespace QM.NFC
{
    ...
    public class NFC_Info
    {
        ...
        public string Payload
        {
            ...
            private set
            {
                if (value == null)
                {
                    _payload = "";
#if DEBUG_LOG
                    Debug.LogFormat("Payload tried to set to null. Used empty string instead.");
#endif
                }
                else
                {
...
#if DEBUG_LOG
            Debug.LogFormat("GQ: NFC_Info: after unmarshalling: Id: {0}\n\tPayload: {1}\n\tTechs: {2}", Id, Payload, Techs);
#endif
        ...
        {
#if DEBUG_LOG
            Debug.LogFormat("GQ: NFC_Info: unmarshalling: found key: {0} with value: {1}", key, value);
#endif
            switch (key)
            {
                case KEY_ID:
#if DEBUG_LOG
                    Debug.LogFormat("GQ: NFC_Info: unmarshalling: set Id to: {0}", value);
#endif
                    Id = value;
                    break;
                case KEY_PAYLOAD:
                    Payload = value;
#if DEBUG_LOG
                    Debug.LogFormat("GQ: NFC_Info: unmarshalling: set Payload to: {0}", value);
#endif
                    break;
                case KEY_TECHS:
                    Techs = value.Split(new string[] { "," }, System.StringSplitOptions.RemoveEmptyEntries);
#if DEBUG_LOG
                    Debug.LogFormat("GQ: NFC_Info: unmarshalling: set Techs to: {0}", value);
#endif
                    break;
                default:
                    ...
                    Valid = false;
#if DEBUG_LOG
                    Debug.LogFormat("GQ: NFC_Info: unmarshalling: Invalid key found: {0} with val: {1}", key, value);
#endif
                    break;
            }
        }
