...
// 
// This file contains a class for processing an input string as hyper text. Its
// primary function is to extract <a> tags and their relevant information from
// the InputText property, and format the resulting string for the OutputText
// property. You can then use the GetLinks() method to get information about the
// links that were found, such as their character indices in OutputText. The
// minimal syntactical requirement for an <a> tag is the "name" attribute. For
// example, the input text "Here is a <a name="some_link">link</a>" will result
// in the output text "Here is a link".
//
// Assigning a HyperTextStyles object to the Styles property also allows for
// some additional processing. If IsRichTextDesired is true, then it will
// automatically convert any custom tags and quads found in InputText, as well
// as insert <color> tags as needed. If IsDynamicFontDesired is true, then
// <size> tags will automatically be inserted for links, custom tags, and quads.
// The value of <size> tags depends on either the font size specified in the
// styles or the ReferenceFontSize property if no styles are assigned, as well
// as the ScaleFactor property. Information about custom tags and quads can then
// be extracted via GetCustomTags() and GetQuads(). The syntactical requirements
...
// 
// You can also assign KeywordCollection objects to automatically detect and
// tag keywords appearing in InputText as either links or custom tags. Any links
// automatically detected in this way will have a name attribute equal to the
// keyword.
// 
// The class also allows specification of sizes as percentages rather than raw
// values. For example, you can use the pattern: "<size=120%>BIG TEXT</size>".
...
namespace Candlelight.UI
{
	...
	public class HyperTextProcessor : System.IDisposable, ITextSource
	{
		...
		/// between keyword collections and styles for <a> tags specified in the style sheet.
		/// </summary>
		[System.Serializable]
		public struct KeywordCollectionClass : IPropertyBackingFieldCompatible
		{
			...
			/// <see cref="Candlelight.UI.HyperTextProcessor+KeywordCollectionClass"/> struct.
			/// </summary>
			...
			/// <summary>
			/// Serves as a hash function for a <see cref="Candlelight.UI.HyperTextProcessor+KeywordCollectionClass"/>
			/// object.
			/// </summary>
			/// <returns>
			...
			public override int GetHashCode()
			{
				return ObjectX.GenerateHashCode(
					string.IsNullOrEmpty(m_ClassName) ? typeof(string).GetHashCode() : m_ClassName.GetHashCode(),
					m_Collection.GetHashCode()
				);
			}
		...
		public class Link : TagCharacterData
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="Candlelight.UI.HyperTextProcessor+Link"/> class.
			/// </summary>
			...
			/// <param name="style">Style.</param>
			public Link(
				string id, string className, IndexRange characterIndices, HyperTextStyles.Link style
			...
			public override object Clone()
			{
				return new Link(this.Id, this.ClassName, (IndexRange)this.CharacterIndices.Clone(), this.Style);
			}
		}
		...
		public class Quad : TagCharacterData
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="Candlelight.UI.HyperTextProcessor+Quad"/> class.
			/// </summary>
			/// <param name="indexRange">Index range.</param>
			/// <param name="style">Style.</param>
		...
		public abstract class TagCharacterData : System.ICloneable
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="Candlelight.UI.HyperTextProcessor+TagCharacterData"/>
			/// class.
			/// </summary>
			/// <param name="indexRange">Index range.</param>
			public TagCharacterData(IndexRange indexRange)
			{
		...
		/// <summary>
		/// A regular expression to extract the attribute value of a <size> tag or the size attribute of a <quad> tag.
		/// </summary>
		...
		/// </summary>
		private static readonly Regex s_PreprocessedLinkTagRegex = new Regex("</?a\b.*?>", RegexOptions.Singleline);
		/// <summary>
		...
		/// </summary>
		private static readonly Regex s_QuadTagRegex =
			new Regex(string.Format("<quad class\\s*=\\s*\"(?<{0}>.+?)\"\\s*.*?/?>", ClassNameCaptureGroup));
		...
		private static readonly ReadOnlyCollection<string> s_ReservedTags = new ReadOnlyCollection<string>(
			new string[] { "a", "b", "color", "i", "material", "quad", "size" }
		);
		#endregion
		...
		private int FontSizeToUse
		{
			get
			{
				return m_ShouldOverrideStylesFontSize || m_Styles == null ?
					ReferenceFontSize : m_Styles.CascadedFontSize;
			}
		}
		/// <summary>
		...
		public GUIStyle GUIStyle
		{
			get
			{
				...
				result.richText = IsRichTextDesired;
				return result;
			}
		}
		/// <summary>
		...
		/// value in the InputText property of this <see cref="Candlelight.UI.HyperTextProcessor"/>.
		/// </summary>
		...
		private Object InputTextSourceObject
		{
			...
			set
			{
				...
				InputTextSource = m_InputTextSourceObject as ITextSource;
			}
		}
		/// <summary>
		...
		{
			get { return InputTextSource != null ? m_InputTextSource.OutputText : m_InputText; }
		}
		/// <summary>
		/// Gets or sets a value indicating whether dynamic font output is desired on this instance.
		/// </summary>
		/// <value><c>true</c> if this dynamic font output is desired on this instance; otherwise, <c>false</c>.</value>
		...
		/// </summary>
		/// <value><c>true</c> if dynamic font output is enabled; otherwise, <c>false</c>.</value>
		...
		/// </summary>
		/// <value><c>true</c> if rich text is desired on this instance; otherwise, <c>false</c>.</value>
		...
		/// </summary>
		/// <value><c>true</c> if rich text is enabled; otherwise, <c>false</c>.</value>
		...
		private int ScaledFontSize { get { return (int)(FontSizeToUse * ScaleFactor); } }
		/// <summary>
		/// Gets or sets a value indicating whether this <see cref="Candlelight.UI.HyperTextProcessor"/> should override
		/// the font size specified in styles, if one is assigned.
		/// </summary>
		...
		/// <summary>
		/// Releases all resource used by the <see cref="Candlelight.UI.HyperTextProcessor"/> object.
		/// </summary>
		/// <remarks>
		/// Call <see cref="Dispose"/> when you are finished using the <see cref="Candlelight.UI.HyperTextProcessor"/>.
		/// The <see cref="Dispose"/> method leaves the <see cref="Candlelight.UI.HyperTextProcessor"/> in an unusable
		/// state. After calling <see cref="Dispose"/>, you must release all references to the
		/// <see cref="Candlelight.UI.HyperTextProcessor"/> so the garbage collector can reclaim the memory that the
		/// <see cref="Candlelight.UI.HyperTextProcessor"/> was occupying.
		/// </remarks>
		public void Dispose()
		{
		...
		[System.Obsolete("Use void HyperTextProcessor.GetLinkKeywordCollections(ref List<KeywordCollection> collections)")]
		public KeywordCollectionClass[] GetLinkKeywordCollections() // TODO: make private
		{
		...
		private string GetPostprocessedQuadTag(Match quadTagMatch, List<HyperTextStyles.Quad> quadTemplates)
		{
			...
			if (templateIndex >= 0)
			{
				...
				if (quadTemplates[templateIndex].Sprite != null)
				{
					...
					rect = quadTemplates[templateIndex].Sprite.rect;
		...
		[System.Obsolete("Use void HyperTextProcessor.GetTagKeywordCollections(ref List<KeywordCollection> collections)")]
		public KeywordCollectionClass[] GetTagKeywordCollections() // TODO: make private
		{
		...
		)
		{
			...
			return regex.Replace(
				...
				string.Format(
					"<a name=\"{0}\"{1}>{2}</a>",
					keyword,
					string.IsNullOrEmpty(className) || !IsRichTextEnabled ?
		...
		private void ProcessInputText()
		{
			...
			s_TextCache = SubstituteTagsInForKeywords(InputTextToUse);
			...
			if (IsRichTextEnabled)
			{
					...
					match => string.Format(
						...
						match.Groups[AttributeValueCaptureGroup].Value.EndsWith("%") ?
							(int)(
								...
								) * ScaledFontSize * 0.01f
							) : (
								(int)float.Parse(match.Groups[AttributeValueCaptureGroup].Value) > 0 ?
								(int)float.Parse(match.Groups[AttributeValueCaptureGroup].Value) : ScaledFontSize
							),
				...
				foreach (HyperTextStyles.Text style in customTags.Values)
				{
					RichTextStyle textStyle =
						IsDynamicFontEnabled ? style.TextStyle : style.TextStyle.NonDynamicVersion;
				...
				// collect link styles
				m_Styles.GetCascadedLinkStyles(ref s_CascadedLinkStyles);
			...
			foreach (Match match in s_PostprocessedLinkTagRegex.Matches(s_TextCache))
			{
				...
				className = string.Empty;
				HyperTextStyles.Link linkStyle = DefaultLinkStyle;
				...
				RichTextStyle textStyle =
					IsDynamicFontEnabled ? linkStyle.TextStyle : linkStyle.TextStyle.NonDynamicVersion;
				s_OpenTag = textStyle.ToStartTag(ScaledFontSize);
				s_CloseTag = textStyle.ToEndTag();
				...
				int startPosition =
					s_ProcessedTextBuilder.Length - (IsRichTextEnabled ? s_CloseTag.Length : 0) - textCapture.Length;
			...
			if (IsRichTextEnabled)
			{
				...
				foreach (Match match in s_QuadTagRegex.Matches(m_OutputText))
				{
					...
					int quadGeomIndex = match.Index + match.Length - 1;
					if (templateIndex >= 0)
					{
						m_Quads.Add(
		...
		private string SubstituteTagsInForKeywords(string input)
		{
			...
			Regex tagMatcher = IsRichTextEnabled ?
				new Regex(
					string.Format(
						"{0}{1}",
		...
		#region Obsolete
		[System.Obsolete("Use void HyperTextProcess.GetCustomTags(ref List<CustomTag> tags)")]
		public CustomTag[] GetCustomTags()
		{
			ProcessInputText();
			return (from customTag in m_CustomTags select (CustomTag)customTag.Clone()).ToArray();
		}
		[System.Obsolete("Use void HyperTextProcess.GetLinks(ref List<Link> links)")]
		public Link[] GetLinks()
		{
			ProcessInputText();
			return (from link in m_Links select (Link)link.Clone()).ToArray();
		}
		[System.Obsolete("Use void HyperTextProcess.GetQuads(ref List<Quad> quads)")]
		public Quad[] GetQuads()
		{
			ProcessInputText();
			return (from quad in m_Quads select (Quad)quad.Clone()).ToArray();
		}
		#endregion
	}
}