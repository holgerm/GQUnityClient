...
namespace Candlelight
{
	...
	public class StatusPropertyAttribute : PropertyAttribute
	{
		...
		public struct Comparison
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="Candlelight.StatusPropertyAttribute.Comparison"/> struct.
			/// </summary>
		...
		/// </summary>
		private static readonly ReadOnlyCollection<System.Type> s_StatusMethodParamTypes =
			new ReadOnlyCollection<System.Type>(
				new System.Type[] { typeof(Object), typeof(object), typeof(string).MakeByRefType() }
			);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Candlelight.StatusPropertyAttribute"/> class to display a
		...
		public StatusPropertyAttribute(ValidationStatus persistentIcon, string iconTooltip, System.Type fieldType)
		{
			BadStatusIcon = persistentIcon;
#if UNITY_EDITOR
			if (fieldType.IsClass) // NOTE: Type.IsClass unavailable in Metro
			{
				m_Comparisons = new Comparison[] { new Comparison(0, null), new Comparison(1, null) };
			}
			else
			{
				object defaultValue = System.Activator.CreateInstance(fieldType);
				m_Comparisons = new Comparison[]
				{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Candlelight.StatusPropertyAttribute"/> class to test for null
		/// values.
		/// </summary>
		...
		{
			BadStatusIcon = badStatusIcon;
			m_Comparisons = new Comparison[] { new Comparison(0, null) };
			this.BadStatusTooltip = GetBadStatusTooltip(badStatusTooltip);
		}
		
		/// <summary>
		/// Initializes a new instance of the <see cref="Candlelight.StatusPropertyAttribute"/> class.
		/// </summary>
		...
		{
			BadStatusIcon = badStatusIcon;
			m_Comparisons = new Comparison[] { new Comparison(badComparisonResult, testValue) };
			this.BadStatusTooltip = GetBadStatusTooltip(badStatusTooltip);
		}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Candlelight.StatusPropertyAttribute"/> class that specifies a
		/// static method to get the status of the decorated field.
		/// </summary>
		...
		/// <see cref="Candlelight.ValidationStatus"/> (<see cref="System.Object"/> provider, <see cref="System.Object"/> testValue, out <see cref="System.String"/> message).
		/// </param>
		public StatusPropertyAttribute(System.Type providerType, string methodName)
		{
#if UNITY_EDITOR
			System.Reflection.MethodInfo method = providerType.GetMethod(
				methodName,
				BindingFlags.FlattenHierarchy | BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public,
				null,
				s_StatusMethodParamTypes.ToArray<System.Type>(),
				null
			);
			if (method == null)
			{
			...
			{
				ValidationMethod = GetStatusMethod.CreateDelegate(typeof(GetStatusMethod), method) as GetStatusMethod;
			}
#endif
		}
		...
		/// <summary>
		/// Gets the comparisons defined on this <see cref="Candlelight.StatusPropertyAttribute"/>.
		/// </summary>
		/// <returns>The comparisons defined on this <see cref="Candlelight.StatusPropertyAttribute"/>.</returns>
		public IEnumerable<Comparison> GetComparisons()
		{
			return m_Comparisons as IEnumerable<Comparison>;
		}
	}
}