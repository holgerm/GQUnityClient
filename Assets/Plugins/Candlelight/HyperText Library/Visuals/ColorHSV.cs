...
namespace Candlelight
{
	...
	{
		/// <summary>
		/// Linearly interpolate the two specified <see cref="Candlelight.ColorHSV"/>s by the specified amount.
		/// </summary>
		/// <returns>A <see cref="Candlelight.ColorHSV"/> interpolated between the two specified values.</returns>
		/// <param name="color1">The first <see cref="Candlelight.ColorHSV"/>.</param>
		/// <param name="color2">The second <see cref="Candlelight.ColorHSV"/>.</param>
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Candlelight.ColorHSV"/> struct.
		/// </summary>
		...
		{
			this.m_Hue = Mathf.Clamp(h, 0f, 360f);
			this.m_Saturation = s;
			this.m_Value = v;
			this.m_Alpha = a;
		}
		...
		public ColorHSV(Color color) : this()
		{
			...
			if (max > 0f)
			{
				this.m_Saturation = delta / max;
			}
			else
			{
				// all colors are zero, no saturation and hue is undefined
				this.m_Saturation = 0f;
				this.m_Hue = -1f;
				return;
			}
			...
			{
				this.m_Hue = (color.g - color.b) / delta;
			}
			else if (color.g == max) // between cyan & yellow
			{
				this.m_Hue = 2f + (color.b - color.r) / delta;
			}
			else // between magenta & cyan
			{
				this.m_Hue = 4f + (color.r - color.g) / delta;
			}
		...
		public Color ToColor()
		{
			...
			float q = v * (1f - Saturation * f);
			float t = v * (1f - Saturation * (1f - f));
			// build rgb color
		...
		/// <summary>
		/// Returns a <see cref="System.String"/> that represents the current <see cref="Candlelight.ColorHSV"/>.
		/// </summary>
		/// <returns>
		/// A <see cref="System.String"/> that represents the current <see cref="Candlelight.ColorHSV"/>.
		/// </returns>
