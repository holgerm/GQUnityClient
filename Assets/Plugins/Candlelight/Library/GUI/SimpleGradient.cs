...
namespace Candlelight.UI
{
...
#endif
	{
		...
		private List<UIVertex> s_Vertices = new List<UIVertex> ();
		#endregion
		...
		[SerializeField]
		private Color32
			m_BottomColor = Color.black;
		[SerializeField]
		private ColorTintMode
			m_ColorTintMode = ColorTintMode.Constant;
		[SerializeField]
		private GradientFillMode
			m_FillMode = GradientFillMode.PerQuad;
		#endregion
...
#endif
		{
			...
			using (UnityEngine.UI.VertexHelper vh = new UnityEngine.UI.VertexHelper(mesh)) {
				vh.GetUIVertexStream (s_Vertices);
			}
			#else
			vh.GetUIVertexStream(s_Vertices);
			#endif
			...
			case GradientFillMode.PerQuad:
				...
				for (int i = 0; i < count; i += 6) {
					ApplyTint (s_Vertices, i, m_TopColor);
					ApplyTint (s_Vertices, i + 1, m_TopColor);
					ApplyTint (s_Vertices, i + 2, m_BottomColor);
					ApplyTint (s_Vertices, i + 3, m_BottomColor);
					ApplyTint (s_Vertices, i + 4, m_BottomColor);
					ApplyTint (s_Vertices, i + 5, m_TopColor);
				}
	#if IS_VBO_MESH
				using (UnityEngine.UI.VertexHelper vh = new UnityEngine.UI.VertexHelper()) {
					vh.AddUIVertexTriangleStream (s_Vertices);
					vh.FillMesh (mesh);
				}
	#else
				vh.Clear();
			...
			case GradientFillMode.GeometryBounds:
				yMin = s_Vertices [0].position.y;
				yMax = s_Vertices [0].position.y;
				for (int i = 1; i < count; i++) {
					float y = s_Vertices [i].position.y;
					if (y > yMax) {
						yMax = y;
					}
					if (y < yMin) {
			...
			{
				ApplyTint(
					vertices, i, Color32.Lerp(m_TopColor, m_BottomColor, (yMax - s_Vertices[i].position.y) * div)
				);
			}
#else
			for (int i = 0; i < count; ++i) {
				ApplyTint (
					s_Vertices, i, Color32.Lerp (m_TopColor, m_BottomColor, (yMax - s_Vertices [i].position.y) * div)
				);
			}
			#if IS_VBO_MESH
			using (UnityEngine.UI.VertexHelper vh = new UnityEngine.UI.VertexHelper()) {
