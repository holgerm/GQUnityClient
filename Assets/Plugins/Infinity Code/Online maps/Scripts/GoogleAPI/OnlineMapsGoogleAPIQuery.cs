...
public class OnlineMapsGoogleAPIQuery
{
    ...
    protected static Vector2 GetVector2FromNode(XmlNode node)
    {
        XmlNode lngNode = node.SelectSingleNode("lng");
        XmlNode latNode = node.SelectSingleNode("lat");

        if (lngNode == null || latNode == null) return Vector2.zero;
        float lng = float.Parse(lngNode.InnerText);
        float lat = float.Parse(latNode.InnerText);
