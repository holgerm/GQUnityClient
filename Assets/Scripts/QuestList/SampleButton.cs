...
public class SampleButton : MonoBehaviour {
	...
	void Start(){
		...
		if (q.hasData) {
			...
			foreach(Image i in	removeButton.GetComponentsInChildren<Image> ()){ i.enabled = true; }

				} else {

			removeButton.enabled = false;
			removeButton.GetComponent<Image>().enabled = false;
			foreach(Image i in	removeButton.GetComponentsInChildren<Image> ()){ i.enabled = false; }
